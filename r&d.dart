...
class CustomCupertinoDatePicker extends StatefulWidget {
  ...
  State<StatefulWidget> createState() { // ignore: no_logic_in_create_state, https://github.com/flutter/flutter/issues/70499
    ...
    switch (mode) {
      ...
      case CustomCupertinoDatePickerMode.yearAndMonth:
        return _CupertinoDatePickerYearMonthState(dateOrder: dateOrder);
    }
  }
...
class _CupertinoDatePickerYearMonthState extends State<CustomCupertinoDatePicker> {

  _CupertinoDatePickerYearMonthState({
    required this.dateOrder,
  });
    ...
    return NotificationListener<ScrollNotification>(
      ...
      child: CupertinoPicker.builder(
        ...
        onSelectedItemChanged: (int index) {
          selectedYear = index;
          if (_isCurrentDateValid) {
            widget.onDateTimeChanged(DateTime(selectedYear, selectedMonth, 0));
          }
        },
        itemBuilder: (BuildContext context, int year) {
          if (year < widget.minimumYear) {
  ...
  bool get _isCurrentDateValid {
    // The current date selection represents a range [minSelectedData, maxSelectDate].
    final DateTime minSelectedDate = DateTime(selectedYear, selectedMonth, 0);
    final DateTime maxSelectedDate = DateTime(selectedYear, selectedMonth, 0 + 1);

    final bool minCheck = widget.minimumDate?.isBefore(maxSelectedDate) ?? true;
    final bool maxCheck = widget.maximumDate?.isBefore(minSelectedDate) ?? false;

    return minCheck && !maxCheck && minSelectedDate.day == 0;
  }
  ...
  void _pickerDidStopScrolling() {
    ...
    // Whenever scrolling lands on an invalid entry, the picker
    // automatically scrolls to a valid one.
    final DateTime minSelectDate = DateTime(selectedYear, selectedMonth, 0);
    final DateTime maxSelectDate = DateTime(selectedYear, selectedMonth, 0 );
